...
def transform_mts_to_ucr_format():
    ...
    mts_root_dir = '/eos/user/l/lfelsber/LASER/TSC_benchmark/data/'
    mts_out_dir = '/eos/user/l/lfelsber/LASER/TSC_benchmark/data/'
...
def calculate_metrics(y_true, y_pred,duration,y_true_val=None,y_pred_val=None):
    res = pd.DataFrame(data = np.zeros((1,10),dtype=np.float), index=[0],
        columns=['precision','accuracy','recall','F1','acc-frac_class_maj','tn', 'fp', 'fn', 'tp','duration'])
    ...
    res['acc-frac_class_maj']=accuracy-max(1-(1-sum(y_true)/len(y_true)),(1-sum(y_true)/len(y_true)))
    try:
        res['tn'], res['fp'], res['fn'], res['tp'] = confusion_matrix(y_true,y_pred).ravel()
    except:
        res['tn'], res['fp'], res['fn'], res['tp'] = [0,0,0,0]
...
def plot_epochs_metric(hist, file_name, metric='loss'):
    ...
    plt.figure()
    plt.semilogy(hist.history[metric])
    plt.semilogy(hist.history['val_'+metric])
    plt.title('model '+metric)
    plt.ylabel(metric,fontsize='large')
    plt.xlabel('epoch',fontsize='large')
    plt.legend(['train', 'val'], loc='upper left')
    plt.savefig(file_name[0:-len(".png")]+"_semilogy.png",bbox_inches='tight')
    plt.close()
